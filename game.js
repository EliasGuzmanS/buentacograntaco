...
function loadCustomer() {
  if (state.customerIndex >= CUSTOMERS.length) {
    endDay();
    return;
  }
  
  state.currentCustomer = JSON.parse(JSON.stringify(CUSTOMERS[state.customerIndex]));
  resetKitchenState();
  
  // Actualizar UI del cliente
  document.getElementById('customer-state-origin').innerText = `${state.currentCustomer.name} (Procedencia: ${state.currentCustomer.state})`;
  document.getElementById('customer-dialog').innerText = `"${state.currentCustomer.dialog}"`;
...
// --- ACTUALIZAR TICKET DE COCINA ---
function updateKitchenTicket(customer) {
  const ticketDiv = document.getElementById('kitchen-ticket');
  ...
  ticketId.innerText = state.customerIndex + 1;
  ticketList.innerHTML = '';
  
  // Tortilla
  const tortillaMap = { 'harina-grande': 'Harina G.', 'harina-chica': 'Harina Ch.', 'maiz': 'Maíz' };
  const liTortilla = document.createElement('li');
  liTortilla.innerHTML = `<span>Tortilla:</span> <strong>${tortillaMap[recipe.tortilla] || recipe.tortilla}</strong>`;
  ticketList.appendChild(liTortilla);
  
  // Untable
  if (recipe.spread) {
    const liSpread = document.createElement('li');
    liSpread.innerHTML = `<span>Base:</span> <strong>${recipe.spread === 'frijoles' ? 'Frijoles' : 'Guacamole'}</strong>`;
    ticketList.appendChild(liSpread);
  }
  
  // Ingredientes
  if (recipe.requiredToppings && recipe.requiredToppings.length > 0) {
    const liToppings = document.createElement('li');
    const tops = recipe.requiredToppings.map(t => {
      if (t === 'asada') return 'Asada';
      if (t === 'machaca') return 'Machaca';
      if (t === 'discada') return 'Discada';
      if (t === 'pescado') return 'Pescado';
      if (t === 'cebolla-cilantro') return 'Cebolla/Cil';
      if (t === 'repollo') return 'Repollo';
      if (t === 'queso') return 'Queso';
      if (t === 'rajas') return 'Rajas';
      if (t === 'salsa-roja') return 'S. Roja';
      if (t === 'salsa-verde') return 'S. Verde';
      if (t === 'limon') return 'Limón';
      return t;
    }).join(', ');
    liToppings.innerHTML = `<span>Relleno:</span> <strong>${tops}</strong>`;
    ticketList.appendChild(liToppings);
  }
  
  // Doblez y Corte
  const liFold = document.createElement('li');
  let foldText = recipe.fold === 'taco' ? 'Taco' : 'Burrito';
  if (recipe.cut) foldText += ' (Cortado)';
  liFold.innerHTML = `<span>Estilo:</span> <strong>${foldText}</strong>`;
  ticketList.appendChild(liFold);
  
  // Notas Especiales
  if (customer.specialMods && customer.specialMods.noVeg) {
    const liMod = document.createElement('li');
    liMod.innerHTML = `<span>Nota:</span> <strong style="color:red">¡SIN VERDURAS!</strong>`;
    ticketList.appendChild(liMod);
  }
  
  // Animación de aparición
  ticketDiv.classList.remove('hidden');
  ticketDiv.style.animation = 'none';
  setTimeout(() => ticketDiv.style.animation = 'ticket-slide-in 0.5s ease-out forwards', 10);
}
  ...
  if (timeout) {
    ...
    targetRecipe.requiredToppings.forEach(top => {
      if (!state.currentOrder.toppings.includes(top)) {
        score -= 20;
        critique.push(`Le faltó ${top === 'asada' ? 'Carne Asada' : (top === 'machaca' ? 'Machaca' : (top === 'discada' ? 'Discada' : (top === 'repollo' ? 'Repollo' : top)))}.`);
      }
    });
    
    // 4. Evaluar Ingredientes Prohibidos / Sobrantes
    // En el caso del vaquero Chema (Durango), no quiere verduras de ningún tipo
    if (customer.specialMods && customer.specialMods.noVeg) {
...
function updateHUD() {
  ...
  document.getElementById('hud-day').innerText = '1';
}
...
function setupSummaryEvents() {
  document.getElementById('btn-restart-day').addEventListener('click', () => {
    if (window.sound) window.sound.playClick();
    // Restablecer dinero inicial por si acaso, para que sea rejugable
    state.money = 100.00;
    startDay();
  });
